 ...
 */
export function jsonToMultipleSheetXlsx<T = any>({ sheetList, filename = DEF_FILE_NAME, write2excelOpts = { bookType: 'xlsx' } }: JsonToMultipleSheet<T>) {
  const workbook: WorkBook = {
    SheetNames: [],
    Sheets: {},
  };
 ...
 * @param write2excelOpts 文件配置
 */
export function aoaToMultipleSheetXlsx<T = any>({ sheetList, filename = DEF_FILE_NAME, write2excelOpts = { bookType: 'xlsx' } }: AoaToMultipleSheet<T>) {
  const workbook: WorkBook = {
    SheetNames: [],
    Sheets: {},
  };
